...
class Author:
    ...
    def full_name(self, author_full_name: str):
        ...
        if type(author_full_name) is str:
            # make sure leading and trailing whitespace is removed
...
class Book:
    ...
    def image_url(self, image_url: str):
        if type(image_url) is str and image_url != "":
    ...
    def release_year(self, release_year: int):
        if type(release_year) is int and release_year >= 0:
    ...
    def num_pages(self, num_pages: int):
        if type(num_pages) is int and num_pages >= 0:
    ...
    def average_rating(self, average_rating: float):
        if type(average_rating) is float or type(average_rating) is int:
            if average_rating > 0 and average_rating <= 5:
...
class Review:
    ...
    def __init__(self, book: Book, review_text: str, rating: int):
        ...
        if type(rating) is int and rating >= 1 and rating <= 5:
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        return other.book == self.book and other.review_text == self.review_text and \
               other.rating == self.rating and other.timestamp == self.timestamp
...
class User:
    ...
    def __init__(self, user_name: str, password: str):
        ...
        if password == "" or type(password) is not str or len(password) < 7:
    ...
    def add_review(self, review: Review):
        if isinstance(review, Review):
            # review objects are always different, due to their timestamp!
...
class BooksInventory:
    ...
    def search_book_by_title(self, book_title: str):
        ...
        for key in self.__books.keys():
            if self.__books[key].title == book_title:
